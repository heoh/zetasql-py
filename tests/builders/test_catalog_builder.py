...
from zetasql.api import CatalogBuilder, TableBuilder, TypeFactory
from zetasql.types import LanguageOptions, SimpleCatalog, TypeKind, ZetaSQLBuiltinFunctionOptions
...
class TestCatalogBuilder:
    ...
    def test_catalog_with_single_table(self):
        ...
        table = (
            TableBuilder("users").add_column("id", TypeKind.TYPE_INT64).add_column("name", TypeKind.TYPE_STRING).build()
        )

        catalog = CatalogBuilder("my_db").add_table(table).build()
    ...
    def test_catalog_with_multiple_tables(self):
        ...
        catalog = CatalogBuilder("shop").add_table(orders).add_table(products).add_table(customers).build()
    ...
    def test_catalog_with_builtin_functions(self):
        ...
        catalog = CatalogBuilder("db").add_table(table).with_builtin_functions(builtin_opts).build()
    ...
    def test_catalog_with_maximum_features(self):
        ...
        builtin_opts = ZetaSQLBuiltinFunctionOptions(language_options=LanguageOptions.maximum_features())

        catalog = CatalogBuilder("db").with_builtin_functions(builtin_opts).build()
    ...
    def test_catalog_method_chaining(self):
        ...
        builtin_opts = ZetaSQLBuiltinFunctionOptions(language_options=LanguageOptions())

        catalog = (
            CatalogBuilder("chained")
            .add_table(t1)
            .add_table(t2)
            .add_table(t3)
...
class TestCatalogBuilderIntegration:
    ...
    def test_complete_catalog_setup(self):
        ...
        customers = (
            TableBuilder("Customers", serialization_id=1)
            .add_column("customer_id", TypeKind.TYPE_INT64)
            .add_column("name", TypeKind.TYPE_STRING)
        ...
        products = (
            TableBuilder("Products", serialization_id=2)
            .add_column("product_id", TypeKind.TYPE_INT64)
            .add_column("name", TypeKind.TYPE_STRING)
        ...
        orders = (
            TableBuilder("Orders", serialization_id=3)
            .add_column("order_id", TypeKind.TYPE_INT64)
            .add_column("customer_id", TypeKind.TYPE_INT64)
            .add_column("product_id", TypeKind.TYPE_INT64)
        ...
        # Build catalog
        catalog = CatalogBuilder("shop").add_table(customers).add_table(products).add_table(orders).build()
    ...
    def test_catalog_with_complex_types(self):
        ...
        tags_array = TypeFactory.create_array_type(TypeFactory.create_simple_type(TypeKind.TYPE_STRING))
