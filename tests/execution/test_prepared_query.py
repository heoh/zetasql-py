...
def catalog(service):
    ...
    from zetasql.types import ZetaSQLBuiltinFunctionOptions
    ...
    catalog = CatalogBuilder("test_db").add_table(orders_table).with_builtin_functions(builtin_opts).build()
...
class TestPreparedQuery:
    ...
    def test_basic_prepare_and_execute(self, service, analyzer_options, catalog, table_content):
        ...
        response = service.prepare_query(
            sql=sql, options=analyzer_options, simple_catalog=catalog, table_content=table_content
        )
...
class TestPreparedQueryBuilder:
    ...
    def test_builder_missing_sql_raises_error(self, analyzer_options, catalog):
        ...
        with pytest.raises(InvalidArgumentError, match="SQL must be set"):
            PreparedQueryBuilder().set_analyzer_options(analyzer_options).set_catalog(catalog).prepare()
    ...
    def test_builder_catalog_conflict_raises_error(self, analyzer_options, catalog):
        ...
        with pytest.raises(InvalidArgumentError, match="Cannot provide both"):
            PreparedQueryBuilder().set_sql("SELECT 1").set_analyzer_options(analyzer_options).set_catalog(
                catalog
            ).set_registered_catalog_id(123).prepare()

    def test_builder_table_content_with_registered_catalog_raises_error(self, analyzer_options, table_content):
        ...
        with pytest.raises(InvalidArgumentError, match="Cannot use table_content with registered catalog"):
            PreparedQueryBuilder().set_sql("SELECT 1").set_analyzer_options(analyzer_options).set_registered_catalog_id(
                123
            ).set_table_content(table_content).prepare()
