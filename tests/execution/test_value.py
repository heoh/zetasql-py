...
def value_catalog(builtin_function_options):
    ...
    test_table = (
        ...
        .add_column("array_col", TypeFactory.create_array_type(TypeFactory.create_simple_type(TypeKind.TYPE_INT64)))
        .build()
    )

    return CatalogBuilder("value_db").add_table(test_table).with_builtin_functions(builtin_function_options).build()
...
class TestComplexValues:
    ...
    def test_struct_value(self):
        ...
        value = Value.struct({"name": Value.string("Alice"), "age": Value.int64(30)})
