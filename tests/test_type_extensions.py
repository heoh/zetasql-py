...
import pytest
from zetasql.types import Type, TypeKind, ArrayType, StructType, StructField
...
class TestTypeExtensions:
    ...
    def test_complex_type_construction(self):
        ...
        name_field = StructField(
            field_name="name",
            field_type=Type(type_kind=TypeKind.TYPE_STRING)
        )
        age_field = StructField(
            field_name="age",
            field_type=Type(type_kind=TypeKind.TYPE_INT64)
        )
