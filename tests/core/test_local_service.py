...
def simple_catalog(analyzer_options):
    ...
    builtin_opts = proto_models.ZetaSQLBuiltinFunctionOptions(language_options=analyzer_options.language_options)

    # Create catalog using CatalogBuilder
    catalog = CatalogBuilder("test").add_table(table).with_builtin_functions(builtin_opts).build()
...
class TestAnalysis:
    ...
    def test_analyze_simple_query(self, service, simple_catalog, analyzer_options):
        ...
        response = service.analyze(
            sql_statement="SELECT 1 AS one", simple_catalog=simple_catalog, options=analyzer_options
        )
    ...
    def test_analyze_table_query(self, service, simple_catalog, analyzer_options):
        ...
        response = service.analyze(
            sql_statement="SELECT id, name FROM TestTable", simple_catalog=simple_catalog, options=analyzer_options
        )
...
class TestExpressions:
    ...
    def test_prepare_simple_expression(self, service, analyzer_options, simple_catalog):
        ...
        response = service.prepare(sql="1 + 2", options=analyzer_options, simple_catalog=simple_catalog)
...
class TestQueries:
    ...
    def test_prepare_simple_query(self, service, simple_catalog):
        ...
        response = service.prepare_query(sql="SELECT 1 AS one", simple_catalog=simple_catalog)
    ...
    def test_prepare_table_query(self, service, simple_catalog):
        ...
        response = service.prepare_query(sql="SELECT id, name FROM TestTable", simple_catalog=simple_catalog)
...
class TestCatalog:
    ...
    def test_register_and_use_catalog(self, service, simple_catalog):
        ...
        query_response = service.prepare_query(sql="SELECT id FROM TestTable", registered_catalog_id=catalog_id)
...
class TestTableExtraction:
    ...
    def test_extract_table_names_simple(self, service):
        ...
        response = service.extract_table_names_from_statement(sql_statement="SELECT * FROM table1")
    ...
    def test_extract_table_names_multiple_statements(self, service):
        ...
        # First statement
        parse_resume = proto_models.ParseResumeLocation(input=sql, byte_position=0)

        response = service.extract_table_names_from_next_statement(parse_resume_location=parse_resume)
