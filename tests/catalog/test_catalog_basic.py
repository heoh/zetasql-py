...
class TestCatalogBasics:
    ...
    def test_catalog_with_multiple_tables(self):
        ...
        table1 = TableBuilder("Table1").add_column("id", TypeKind.TYPE_INT64).build()

        table2 = TableBuilder("Table2").add_column("name", TypeKind.TYPE_STRING).build()

        table3 = TableBuilder("Table3").add_column("value", TypeKind.TYPE_DOUBLE).build()

        catalog = CatalogBuilder("db").add_table(table1).add_table(table2).add_table(table3).build()
...
class TestCatalogBuiltinFunctions:
    ...
    def test_catalog_without_builtin_functions(self):
        ...
        assert (
            not hasattr(catalog, "builtin_function_options")
            or catalog.builtin_function_options is None
            or not catalog.builtin_function_options.language_options.enabled_language_features
        )
    ...
    def test_get_builtin_functions_from_service(self, service):
        ...
        [f.name_path[0] for f in response.function if f.name_path]
