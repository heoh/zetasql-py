...
class TestComplexTypeAnalysis:
    ...
    def test_array_operations(self, options, builtin_function_options):
        ...
        table = (
            ...
            .add_column("tags", TypeFactory.create_array_type(TypeFactory.create_simple_type(TypeKind.TYPE_STRING)))
            .build()
        )

        catalog = CatalogBuilder("db").add_table(table).with_builtin_functions(builtin_function_options).build()
    ...
    def test_struct_operations(self, options, builtin_function_options):
        ...
        table = (
            TableBuilder("users").add_column("user_id", TypeKind.TYPE_INT64).add_column("address", address_type).build()
        )
