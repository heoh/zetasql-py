...
from zetasql.api import TableBuilder, CatalogBuilder
    ...
    if isinstance(node, ProtoModel) and hasattr(node, '__dataclass_fields__'):
        ...
        for field in fields(node):
            ...
            if isinstance(field_value, list):
                ...
                lines.append(f"{indent}  └─ {field_name}: {repr(field_value)}")
...
class TestExecuteQueryModes:
    ...
    def test_parse_mode(self, service):
        ...
        assert isinstance(response.parsed_statement, ASTQueryStatement), \
            "Top-level node should be ASTQueryStatement"
        ...
        assert isinstance(select_col.expression, ASTBinaryExpression), \
            "Expression should be binary (1+2)"
    ...
    def test_analyze_mode(self, service, mini_simple_catalog, options):
        ...
        response = service.analyze(
            sql_statement=sql,
            simple_catalog=mini_simple_catalog,
            options=options
        )
        ...
        assert isinstance(stmt.query, ResolvedProjectScan), \
            "Top scan should be ResolvedProjectScan (for SELECT list)"
        ...
        print(f"  • Scan chain: ProjectScan → FilterScan → TableScan")
    ...
    def test_execute_mode(self, service, mini_simple_catalog, options, mini_table_content):
        ...
        prepare_response = service.prepare_query(
            ...
            simple_catalog=mini_simple_catalog,
            options=options,
            table_content=mini_table_content
        )
    ...
    def test_unanalyze_mode(self, service, mini_simple_catalog, options):
        ...
        re_analyze_response = service.analyze(
            sql_statement=regenerated_sql,
            simple_catalog=mini_simple_catalog,
            options=options
        )
        
        assert re_analyze_response.resolved_statement is not None, \
            "Regenerated SQL should be valid and analyzable"
        ...
        assert original_cols == regenerated_cols, \
            f"Output columns should match: {original_cols} vs {regenerated_cols}"
        ...
        print(f"  • Regenerated SQL is valid: ✓")
