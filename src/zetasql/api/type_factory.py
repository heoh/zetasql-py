...
from zetasql.types import Type, ArrayType, StructType, StructField, MapType, TypeKind
...
class TypeFactory:
    ...
    @staticmethod
    def create_simple_type(type_kind: Union[TypeKind, int]) -> Type:
    ...
    def create_array_type(element_type: Type) -> Type:
        ...
        """
        return Type(
            type_kind=TypeKind.TYPE_ARRAY,
            array_type=ArrayType(element_type=element_type)
        )
    ...
    def create_struct_type(fields: List[Tuple[str, Type]]) -> Type:
        ...
        """
        struct_fields = [
            StructField(field_name=name, field_type=field_type)
            for name, field_type in fields
        ]
        return Type(
            type_kind=TypeKind.TYPE_STRUCT,
            struct_type=StructType(field=struct_fields)
        )
    ...
    def create_map_type(key_type: Type, value_type: Type) -> Type:
        ...
        """
        return Type(
            type_kind=TypeKind.TYPE_MAP,
            map_type=MapType(key_type=key_type, value_type=value_type)
        )
